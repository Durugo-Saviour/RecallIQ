...
`).join('\n')
    ...
    try {
      ...
      const prompt = `You are a study assessment tool. Grade the student's answers. Return ONLY a valid JSON array, no markdown, no explanation.
...
[
  { "score": <0-100>, "feedback": "<one sentence of constructive feedback>" }
]`
      ...
      throw new Error("Failed to evaluate answers. Please check your Gemini API key or try again.")
    }
    ...
    const newMastery = Math.round((oldMastery * 0.7) + (sessionScore * 0.3))
